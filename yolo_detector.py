...
class yolo_inference:
    def __init__(
        ...
        if self.not_indexes is None:
            self.indexes = np.linspace(0, self.frame_count - 2, self.steps, dtype=int)
            self.iterated_frames = self.indexes.tolist()
        else:
            frame_seq = list(range(self.frame_count - 2))
            frame_seq = list(set(frame_seq) - set(not_indexes))
            frame_idx = list(np.linspace(0, len(frame_seq) - 2, steps, dtype=int))
            self.indexes = [frame_seq[i] for i in frame_idx]
            self.iterated_frames = not_indexes + self.indexes
    
        self.save_file_name = file_name[0:file_name.find('.')]
    ...
    def detect_and_predict_image(self):
      ...
      while i < len(indexes) and frames_with_objects < find_n_frames:
        frame_num = indexes[i]
        frame, time_second = extract_frame(cap_obj, frame_num)
        if frame is None:
            i +=1
        ...
        if detection_metadata:
           ...
           crop_and_save_image(frame, detection_metadata, self.save_file_name, frame_num, classifier_model = self.classifier_model, return_dict=True, save_dir = save_dir_path, full_image = True)
           write_timestamps(self.save_file_name, frame_num, time_second)
